...
static void polar(const Vector3d& EPoint, const TNORMAL* Tnormal, Vector3d& Vector, const TraceThreadData* Thread);
...
static void polar(const Vector3d& EPoint, const TNORMAL* Tnormal, Vector3d& normal, const TraceThreadData* Thread)
{
	//TO DO FUNCTION
}
...
void Perturb_Normal(Vector3d& Layer_Normal, const TNORMAL *Tnormal, const Vector3d& EPoint, Intersection *Intersection, const Ray *ray, TraceThreadData *Thread)
{
    ...
    if (Tnormal->Type <= LAST_SPECIAL_NORM_PATTERN)
    {
        ...
        switch (Tnormal->Type)
        {
			...
			case POLAR_PATTERN:     polar       (TPoint, Tnormal, Layer_Normal, Thread);    break;
