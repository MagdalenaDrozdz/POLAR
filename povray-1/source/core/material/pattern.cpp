...
* CHANGES
*
*   May 2020 : adapted from normal by [CY]
*
******************************************************************************/
...
{
	double magnitude = sqrt((EPoint[0] * EPoint[0]) + (EPoint[1] * EPoint[1]));
	double theta = atan(EPoint[1] / EPoint[0]);
	std::complex<double> result = std::polar(magnitude, theta);
	return abs(arg(result));
}
